...
class PyflowPipeline(WorkflowRunner):
	...
	def workflow(self):
		...
		taskname = "fastqc"
		dependencies=()
		fastqc="fastqc -o %s %s"%(output_directory, f)
       		self.addTask(taskname,command=fastqc,dependencies=dependencies)


### Check input directory. Get list of samples/folders and fastq files within. For each  sample execute workflow on fastq files
